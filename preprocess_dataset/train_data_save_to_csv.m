%Save train and test datasets information to csv file
...
%% 2. Load all the three datasets and shuffle them

text_read = fileread('synthetic_train_enriched.json');
image_data=jsondecode(text_read);


%% 3. If you are preparing the dataset for a domain agnostic training, 
% comment this section as you are not going to need test images for training
% if instead you are preparing the dataset for adversarial domain
% adaptation, you shall also augment sunlamp and lightbox json files
...
for i=1:length(image_data)
    ...
    % if the class is not 1 (synthetic) we will export zero values
    ...
    if class == 1
        ...
        %bounding boxes regressed by the ODN, move the next 4 lines
